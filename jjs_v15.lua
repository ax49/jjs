...
-- Auto Perfect Block + Moveset-aware Auto Counter.
-- Chara removed (no longer in game).
...
-- =====================
local CONFIG = {
    autoQTE = { enabled = true, reactionDelay = 0, postPressDelay = 0.30, deviation = 0 },
    ...
    autoPerfectBlock = { enabled = false, range = 10, cooldown = 0.5, reactDelay = 0.05 },
    autoCounter = { enabled = false, range = 10, cooldown = 1.0, reactDelay = 0.03 },
}

-- =====================
-- COUNTER MOVE TABLE (wiki names)
-- =====================
-- Maps moveset name -> VK keycode of counter slot
-- Vessel (Yuji): 4 (Manji Kick)
-- Aspiring Mangaka: 3
-- Restless Gambler (Hakari): R (Counter / Door Guard)
-- Blood Manipulator (Choso): 3
-- Anything not listed: skip (no counter)
local COUNTER_MOVES = {
    ["Vessel"] = 0x34,             -- 4
    ["Aspiring Mangaka"] = 0x33,   -- 3
    ["Restless Gambler"] = 0x52,   -- R
    ["Blood Manipulator"] = 0x33,  -- 3
}

-- Case-insensitive substring matches on AnimationTrack.Name
local WINDUP_PATTERNS = {
    "m1", "strike", "kick", "punch", "swing", "slash",
    "dismantle", "cleave", "grapple", "rush", "blitz",
    "rapid", "lapse", "reversal", "focus", "divergent",
    "brute", "swift", "elbow", "pebble", "clap",
    "ratio", "sacrilege", "ambush", "backstab",
    "garuda", "stockpile", "rising", "crushing", "takedown",
}
...
local QTEState = {detectedKey = "", detectedTime = 0, displayDuration = 1.0, lastLabelText = ""}
...
local PerfectBlockState = { lastTriggerTime = 0, lastDetectionTime = 0 }
...
local function HasBlackflashMoveset()
    local m = GetMovesetAttr()
    return m == "Itadori" or m == "Mahito"
end
...
local function ProcessQTE()
    ...
    -- Debounce: only react to TEXT TRANSITIONS, not every loop tick
    if labelText == QTEState.lastLabelText then return false end
...
-- =====================
-- ENEMY WINDUP DETECTION (shared by Auto Block + Auto Counter)
-- =====================
-- Returns the first enemy character within `range` studs that is currently
-- playing an offensive windup animation. Nil otherwise.
local function FindThreateningEnemy(range)
    ...
    for _, char in ipairs(characters:GetChildren()) do
        if char.Name ~= myName and char.Name ~= "Dummy" then
                ...
                if dist <= range then
                    ...
                    if ok and tracks then
                        for _, track in ipairs(tracks) do
                            if StringContainsAny(track.Name, WINDUP_PATTERNS) then
                                local len = track.Length or 0
                                local pos = track.TimePosition or 0
                                -- Only react to early windups (< 55% through animation)
                                if len <= 0 or (pos / len) < 0.55 then
                                    return char
                                end
                            end
                        end
...
-- AUTO PERFECT BLOCK
-- =====================
local function ProcessAutoPerfectBlock()
    if not CONFIG.autoPerfectBlock.enabled then return end
    local now = GetTime()
    if now - PerfectBlockState.lastTriggerTime < CONFIG.autoPerfectBlock.cooldown then return end

    -- Don't override the user if they're already holding F
    if iskeypressed(KEYS.F) then return end

    if not FindThreateningEnemy(CONFIG.autoPerfectBlock.range) then return end
    PerfectBlockState.lastDetectionTime = now

    if CONFIG.autoPerfectBlock.reactDelay > 0 then
        task.wait(CONFIG.autoPerfectBlock.reactDelay)
    end

    keypress(KEYS.F)
    task.wait(0.08)
    keyrelease(KEYS.F)
    PerfectBlockState.lastTriggerTime = GetTime()
end

-- =====================
-- AUTO COUNTER (moveset-aware)
-- =====================
local function ProcessAutoCounter()
    if not CONFIG.autoCounter.enabled then return end
    local moveset = GetMovesetAttr()
    if not moveset then return end

    local counterKey = COUNTER_MOVES[moveset]
    if not counterKey then return end -- moveset has no counter, skip

    local now = GetTime()
    if now - CounterState.lastTriggerTime < CONFIG.autoCounter.cooldown then return end

    if iskeypressed(counterKey) then return end

    if not FindThreateningEnemy(CONFIG.autoCounter.range) then return end
    CounterState.lastDetectionTime = now

    if CONFIG.autoCounter.reactDelay > 0 then
        task.wait(CONFIG.autoCounter.reactDelay)
    end

    keypress(counterKey)
    task.wait(0.05)
    keyrelease(counterKey)
    CounterState.lastTriggerTime = GetTime()
end
...
local function GetStatus()
    ...
    if CONFIG.autoPerfectBlock.enabled then
        local recent = (GetTime() - PerfectBlockState.lastTriggerTime) < 0.6
        table.insert(parts, recent and "Block: HIT" or "Block: ON")
    end
...
-- UI SETUP
-- =====================
local myGui = UILib.new("JJS", Vector2.new(300, 560), {GetStatus})
local running = true
...
myGui:Checkbox(autoTab, blockSection, "Enable", CONFIG.autoPerfectBlock.enabled, function(s) CONFIG.autoPerfectBlock.enabled = s end)
...
myGui:Slider(autoTab, blockSection, "React Delay", CONFIG.autoPerfectBlock.reactDelay * 1000, function(v) CONFIG.autoPerfectBlock.reactDelay = v / 1000 end, 0, 200, 5, "ms")
...
myGui:Checkbox(autoTab, counterSection, "Enable", CONFIG.autoCounter.enabled, function(s) CONFIG.autoCounter.enabled = s end)
...
spawn(function() while running do pcall(ProcessAutoPerfectBlock) task.wait(0.02) end end)
spawn(function() while running do pcall(ProcessAutoCounter) task.wait(0.02) end end)
